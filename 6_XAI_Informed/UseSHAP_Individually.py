...
# for ivar in [0]:
    ...
    ##### LOAD SHAP #####
    shap_correct_pos = np.load(f'FilesIndTests/{list_vars[ivar]}_{week_out_str}_shap_correct_pos.npy')[:,:-2]
    shap_correct_neg = np.load(f'FilesIndTests/{list_vars[ivar]}_{week_out_str}_shap_correct_neg.npy')[:,:-2]
    where_x = np.where((abs(shap_correct_pos).mean(axis=0)>0)|\
                      (abs(shap_correct_neg).mean(axis=0)>0))[0]
    combined_df = combined_df.iloc[:,where_x]
...
np.save(f'ResultsXAI_Informed/{week_out_str}_best_params.npy',best_params)
np.save(f'ResultsXAI_Informed/{week_out_str}_scores_full.npy',scores_full)
np.save(f'ResultsXAI_Informed/{week_out_str}_random_params.npy',random_params)
np.save(f'ResultsXAI_Informed/{week_out_str}_metrics_iterations.npy',metrics_iterations)
df_results.to_csv(f'ResultsXAI_Informed/{week_out_str}_df_results.csv')
df_probabilities.to_csv(f'ResultsXAI_Informed/{week_out_str}_df_probabilities.csv')